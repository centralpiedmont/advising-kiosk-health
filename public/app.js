...
const SCREENS = {
  attract: () => el(`
    <section class="screen attract">
      <div class="topbar"><img src="assets/logo-white.png" alt="Central Piedmont"><div class="it">Health Sciences</div></div>
      <div class="center">
        <div class="rule"></div>
        <h1>Conquer <span class="g">Possibility</span></h1>
        <p class="sub">Explore 16 Health Sciences programs and find yours in three taps.</p>
  ...
  world: () => {
    ...
    for (const w of data.worlds) {
      ...
      const t = el(`<button class="tile tile-${w.id}" style="${wBg};color:${w.text};text-shadow:${w.text === '#FFFFFF' ? '0 2px 10px rgba(0,0,0,.45)' : 'none'}"><span class="tile-name">${esc(w.name)}</span><span class="tile-desc">${esc(w.desc)}</span></button>`);
...
function ceFooter(qrFile, bigText) {
  const band = el(`<div class="foot">
    ...
    <button class="info">Find a Health Sciences info session</button>
  </div>`);
...
function ceView() {
  ...
  const cols = ce.categories.map((cat) => `
    <div class="term"><div class="th"><b>${esc(cat.name)}</b><span>${cat.courses.length}</span></div>
      <div class="rows">${cat.courses.map((c) => `<button class="row ce-row" data-code="${esc(c.code)}"><div class="nm">${esc(c.name)}</div>${c.price ? `<div class="ce-meta">$${c.price}${c.hours ? ` &middot; ${c.hours} hrs` : ''}</div>` : `<div class="ce-meta tba">Tap to view &amp; register &rarr;</div>`}</button>`).join('')}</div></div>`).join('');
  const s = el(`
    <section class="screen detail ce">
      ...
      <div class="titlebar"><span class="fam" style="background:${ce.color};color:${ce.text}">${esc(ce.short)}</span>
        <h2>Certifications &amp; Short Courses</h2><div class="tap-hint">Tap any course for details</div></div>
...
function ceCourseView(c) {
      ...
      <div class="body">
        <div class="left">
          ...
          <div class="meta">
            ...
            <div class="facts">
              ${c.price
                ? `<div><b>$${c.price}</b><small>Course fee</small></div><div><b>${c.hours || 'TBA'}</b><small>Contact hours</small></div><div><b>${esc(c.code)}</b><small>Course code</small></div>`
                : `<div><b>Non-credit</b><small>Continuing Education</small></div><div><b>In person or online</b><small>Flexible formats</small></div>`}
            </div>
          </div>
        </div>
        <div class="right">
  ...
  s.querySelector('.restart').onclick = () => state.startOver();
  s.querySelector('.backbtn').onclick = () => state.goBack();
  s.appendChild(ceFooter(c.qrFile || ce.qrFile, c.qrFile ? 'Scan to view this program &amp; register' : 'Scan to browse dates, pricing &amp; register'));
  return s;
}
  ...
  const s = el(`
    <section class="screen quiz quizr" style="--wc:${world.color};--wcd:${darken(world.color, 0.5)};--wt:${world.text}">
      <div class="reveal">
        ...
        <div class="r-info">
          <div class="r-eyebrow">YOUR HEALTH SCIENCES MATCH</div>
...
const OVERLAYS = {
  infoQR: () => {
    const o = el(`<div class="overlay"><div class="modal">
      <h3>Find a Health Sciences info session</h3>
